...
'''
Usage:
$ spark-submit --driver-memory=8g --executor-memory=8g --conf "spark.blacklist.enabled=false" param_train_2nd.py hdfs:/user/zz4140/train_index_downsample.parquet hdfs:/user/zz4140/1004-project-2023/interactions_val.parquet hdfs:/user/zz4140/indexer_downsample.parquet
'''
...
def main(spark, train_path, val_path, indexer_model):
    ...
    rank_val = []#the depends on single param train: [10, 20, 30] #default is 10
    reg_val = []#the depends on single param train: [ .01, .1, 1] #default is 1
    alpha_val = []#the depends on single param train: [ 1, 10, 20] #default is 1
